...
class MinimalCryptoEnv(gym.Env):
    ...
    def _get_obs(self):
        ...
        # After computing holdings_pct, also compute per-asset unrealised PnL %
    ...
    def step(self, action):
        ...
        if self.reward_type == "excess_return":
            ...
            if not done and current_asset_value > 0:
                hold_cost = current_asset_value * self.hold_cost_rate  # configurable hold cost per step
                reward -= hold_cost
        else:
        ...
        return (
            ...
            False,
            {"fees_paid": self.fees_paid_total, "trades": self.trades_count},
        )
  ...
  python minimal_rl.py --dashboard --rows 15000 --timesteps 50000 --run-dir rl_dashboard_runs
    ...
    def read_last_n(path, n=10000):
        ...
        if ds is None or pq is None:
            ...
            df_local = df_sub[
                (df_sub["open_time"] >= t_start) & (df_sub["open_time"] <= t_end)
            ]
            return df_local.sort_values(by=["open_time", "symbol"]).reset_index(
                drop=True
            )
        ...
        def _collect_metrics(self):
            ...
            try:
                ...
                self.series["portfolio_value"].append(
                    {"step": step, "value": current_portfolio}
                )
                ...
                self.series["drawdown_pct"].append(
                    {"step": step, "value": float(drawdown)}
                )
                ...
                self.series["policy_loss"].append(
                    {
                        "step": step,
                        "value": float(
                            logger_map.get("train/policy_gradient_loss", 0.0)
                        ),
                    }
                )
                self.series["value_loss"].append(
                    {
                        "step": step,
                        "value": float(logger_map.get("train/value_loss", 0.0)),
                    }
                )
                self.series["approx_kl"].append(
                    {
                        "step": step,
                        "value": float(logger_map.get("train/approx_kl", 0.0)),
                    }
                )
                ...
                state = {
                    "run": {
                        ...
                        "progress_pct": int(
                            100
                            * min(
                                1.0, float(self.num_timesteps) / float(args.timesteps)
                            )
                        ),
                    },
                    "technical": {
                        "loss": {
                            "train": self.series["train_loss"][-1]["value"]
    ...
    eval_win_rate_pct = (
        (eval_winning_trades / eval_trades_count * 100.0)
        if eval_trades_count > 0
        else 0.0
    )
